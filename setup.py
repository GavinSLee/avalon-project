...
class Setup:
    ...
    def five_players(self):
        good_roles = ["KNIGHT OF ARTHUR",
                      "KNIGHT OF ARTHUR", "KNIGHT OF ARTHUR"]
    ...
    def six_players(self):
        good_roles = ["KNIGHT OF ARTHUR", "KNIGHT OF ARTHUR",
                      "KNIGHT OF ARTHUR", "KNIGHT OF ARTHUR"]
    ...
    def assign_roles(self):
        ...
        self.visible_to_merlin = {key: value for (key, value) in self.assignments.items(
        ) if value == "MINION OF MORDRED" or value == "MORGANA"}
        # Roles visible to Percival
        self.visible_to_percival = {key: value for (
            key, value) in self.assignments.items() if value == "MERLIN" or value == "MORGANA"}

    def send_roles(self):
        # Loop through friends list
        starting_player = self.friends_list[random.randint(
            0, len(self.friends_list) - 1)]
        ...
        for friend in self.friends_list:
            ...
            msg_intro = "Hi " + \
                str.split(friend)[
                    0] + "!" + " This is a python script written by Gavin for Resistance: Avalon. Welcome to today's game!"
            print(msg_intro)
            # client.send(fbchat.models.Message(msg_intro), name.uid)
            time.sleep(3.0)
            msg_num_people = "There are " + \
                str(len(self.friends_list)) + " people playing this round."
            ...
            if role == "KNIGHT OF ARTHUR":
                msg_knight_arthur = "YOUR ROLE: " + role + \
                    ". Seek fellow knights and defeat evil!"
            ...
            # Message for Merlin
                ...
                msg_merlin = "YOUR ROLE: " + role + ". The following players are evil: " + \
                    ', '.join(key for key in visible_to_merlin.keys())
                print(msg_merlin)
                # self.client.send(fbchat.models.Message(msg_merlin), name.uid)
            # Message for Percival
            elif role == "PERCIVAL":
                msg_percival = "YOUR ROLE: " + role + ". The following may be Merlin: " + \
                    ", ".join(key for key in visible_to_percival.keys())
                print(msg_percival)
                # self.client.send(fbchat.models.Message(msg_percival), name.uid)

            # Message for Vanilla Evil
            elif role == "MINION OF MORDRED":
                msg_minion_mordred = "YOUR ROLE: " + role + \
                    ". Seek fellow minions and defeat good!"
                print(msg_minion_mordred)
                # self.client.send(fbchat.models.Message(msg_minion_mordred), name.uid)

            # Message for Morgana
            elif role == "MORGANA":
                msg_morgana = "YOUR ROLE: " + role + \
                    ". You appear to Percival as a clone of Merlin!"
                print(msg_morgana)
